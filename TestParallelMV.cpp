...
int main(int argc, char* argv[]) {
    ...
    int arrSize[]={10,100,1000,2000,3000,4000,5000,6000,7000,8000,9000,10000};
    MPI_Init(&argc, &argv);
    ...
    for(int i=0;i<12;i++){
        if(ProcRank==0) printf("Matrix, vecotr size: %d\n",arrSize[i] );
    ...
    DataDistribution(pMatrix, pProcRows, pVector, arrSize[i],RowNum);

    // Parallel matrix-vector multiplication
    ParallelResultCalculation(pProcRows, pVector, pProcResult, arrSize[i],RowNum);

    // Result replication
    ResultReplication(pProcResult, pResult, arrSize[i], RowNum);

    Finish = MPI_Wtime();
    Duration = Finish-Start;
   TestResult(pMatrix, pVector, pResult, arrSize[i]);
